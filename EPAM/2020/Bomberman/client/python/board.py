...
class Bomb:
    ...
    def __init__(self, id, owner, blast, timer, btype):
        self.ID = id
        self.blast = blast
        self.timer = timer
...
class GameState:
    ...
    def me_safe_to_drop_bomb(self):
        ...
        #return not self.if_on_any_bombs_fireline(mx, my)
    ...
    def count_objects_around(self, x, y, t):
        res=0
        for dx,dy in MOVES[:4]: 
            if self.is_onboard(x+dx, y+dy):
                if self.BOARD[y+dy][x+dx] in t:
                    res+=1
        return res
        ...
        for i, (x,y) in enumerate(objects_bombermans_xy):
            ...
            if b==None: #new bomb
                ...
                if p!=None: #found player
                    ...
                    nb = self.new_bomb( x, y, -1, p.CONFIG["bombPower"], 5-1, BOMBTYPE_UNKNOWN )
    ...
    def dmap_put_bombs(self):
        for i,bomb in self.bombs().items():
            ...
            y = bomb.Y
            t = bomb.timer
            if (bomb.bomb_type == BOMBTYPE_REMOTE or bomb.timer==5) and bomb.owner!=0:
                t=1
            # if (bomb.bomb_type == BOMBTYPE_REMOTE or bomb.timer==5) and bomb.owner==0:
            #     t=2 #to force me move out from line 
            for dx, dy in MOVES[:4]:
    ...
    def dmap_put_player(self, x, y, target, dist, dw, allowed_cells):
        V = [ [0]*self.BOARD_SZ for _ in range(self.BOARD_SZ)]
        M = deque()
        M.append( (x,y,0) )
    ...
    def dmap_put_choppers(self):
        ...
        if tlist!=None:
            for x,y in tlist:
                self.dmap_put_chopper(x,y,EL_MEAT_CHOPPER, 3, 0, ELS_NOT_DANGER2MOVE+[EL_OTHER_BOMBERMAN, EL_OTHER_BOMB_BOMBERMAN, EL_BOMB_BOMBERMAN])
    ...
    def dmap_put_players(self):
        tlist = self.OBJECTS.get(EL_OTHER_BOMBERMAN, None) #board_get_objects(borig, [EL_MEAT_CHOPPER])
        if tlist!=None:
            for x,y in tlist:
                self.dmap_put_player(x,y,EL_OTHER_BOMBERMAN, 2, 0, ELS_NOT_DANGER2MOVE+[EL_OTHER_BOMBERMAN, EL_OTHER_BOMB_BOMBERMAN, EL_BOMB_BOMBERMAN])
        tlist = self.OBJECTS.get(EL_OTHER_BOMB_BOMBERMAN, None)
        if tlist!=None:
            for x,y in tlist:
                self.dmap_put_player(x,y,EL_OTHER_BOMB_BOMBERMAN, 5, 1, ELS_NOT_DANGER2MOVE+[EL_OTHER_BOMBERMAN, EL_OTHER_BOMB_BOMBERMAN, EL_BOMB_BOMBERMAN])
    ...
    def fill_dangers_map(self):
        ...
        for x in range(self.BOARD_SZ):
            for y in range(self.BOARD_SZ):
                if self.BOARD[y][x] in ELS_NOT_DANGER2MOVE:
                    cnt = self.count_objects_around( x,y,ELS_NO_MOVE_EXCEPT_ME )
                    t = EL_NONE
                    #if cnt==2:
                    #    t = EL_BOMB_BLAST_4
                    if cnt==3:
                    ...
                    if t!=EL_NONE:
                        self.DANGERS[y][x]=t
                        self.DANGERS_t[y][x]=DANGERS_TYPE_BLAST

        self.dmap_put_bombs()
        self.dmap_put_choppers()
        self.dmap_put_players()
    ...
    def if_on_my_rc_fireline(self, px, py):
        ...
        if my_rc==None:
            return
        return self.if_on_fireline( px, py, my_rc.X, my_rc.Y, my_rc.blast + 1)
    ...
    def if_on_any_bombs_fireline(self, px, py):
        for b in self.bombs().values():
            res=self.if_on_fireline( px, py, b.X, b.Y, b.blast )
            if res:
                return True
    ...
    def board_object_remove(self, x, y, t):
    ...
    def board_object_replace(self, x, y, told, tnew):
        ...
        self.board_object_remove( x,y,told )
