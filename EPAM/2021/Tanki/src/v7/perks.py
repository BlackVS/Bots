...
class Perk:
    ...
    def __init__(self, X, Y, ptype):
        ...
        self.perk_live = CONFIG["prizeOnField"]
...
class Perks:
    ...
    def perk_could_be_masked(self, x, y):
        perk = self.perks.get( (x,y) ) 
        if perk!=None: #found just set updated
            perk.updated = True

    def perk_found(self, x, y, ptype):
        perk = self.perks.get( (x,y) )
        if perk!=None: #found< just set updated
            if ptype!=EPRIZE:
                perk.perk_type=ptype
            perk.updated = True
            #perk.not_updated_cnt = 0
        else:
            self.perks[(x,y)] = Perk(x,y,ptype)
            self.perks[(x,y)].updated = True
            #self.perks[(x,y)].not_updated_cnt = 0
